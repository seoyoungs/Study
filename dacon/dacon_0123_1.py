#####  다시 해보자구~~~!!도전!!! >o< ^o^
### GHI, Td, Td-d
### 상관관계 후 WS 버리기, T도 버리자 상관성 0.5로 낮다 두번째로
#### 데이터를 더 쪼갤 수 있을까~~???
...
def preprocess_data(data, is_train=True):
    ...
    temp = data.copy()
    temp = temp[['Hour','TARGET','GHI','T-Td','DHI','DNI','RH']] # 상관관계 낮은 WS, T 삭제
    ...
    if is_train==True:          
        ...
        temp = temp[['Hour','TARGET','GHI','T-Td','DHI','DNI','RH']]
...
print(x_train.shape,x_val.shape) #(41971, 1, 7) (10493, 1, 7)
print(y1_train.shape, y1_val.shape, y2_train.shape, y2_val.shape) #(41971, 1) (10493, 1) (41971, 1) (10493, 1)
...
lr = ReduceLROnPlateau(monitor = 'val_loss', patience = 7, factor = 0.25, verbose = 1)
epochs = ep = 200
batch_size = ba = 32
...
submission.to_csv('C:/data/dacon_data/sub_0123_2.csv', index = False)
