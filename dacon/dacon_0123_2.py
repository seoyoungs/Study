####이건 x,y 를 각각 2개씩 나눠보기
## split_xy --> x1,x2,y1,y2
#### temp 에 T 넣기, RH 삭제해보기
...
def preprocess_data(data, is_train=True):
    ...
    temp = data.copy()
    temp = temp[['Hour','TARGET','GHI','T-Td','DHI','DNI', 'T']] # 상관관계 낮은 WS, T 삭제
    ...
    if is_train==True:          
        ...
        temp = temp[['Hour','TARGET','GHI','T-Td','DHI','DNI', 'T']]
...
x_train= x_train.reshape(36724*1, 7)
x_val= x_val.reshape(15740*1, 7)
...
for i in q:
    model = modeling()
    filepath_cp = f'C:/data/modelCheckpoint/dacon_y1_0123_1_quantile_{i:.1f}.hdf5'
...
for i in q:
    model = modeling()
    filepath_cp = f'C:/data/modelCheckpoint/dacon_y2_0123_1_quantile_{i:.1f}.hdf5'
    cp = ModelCheckpoint(filepath_cp,save_best_only=True,monitor = 'val_loss')
    model.compile(loss = lambda y_true,y_pred: quantile_loss(i,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(i,y,y_pred)])
...
submission.to_csv('C:/data/dacon_data/sub_0123_3.csv', index = False)
