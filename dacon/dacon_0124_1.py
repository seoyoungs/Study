...
def preprocess_data(data, is_train=True):
    ...
    temp = data.copy()
    temp = temp[['Hour_Minute','ordering','TARGET','GHI','T-Td','DHI','DNI', 'RH','T']] # 상관관계 낮은 WS, T 삭제
    ...
    if is_train==True:          
        ...
        temp = temp[['Hour_Minute','ordering','TARGET','GHI','T-Td','DHI','DNI', 'RH','T']]
...
x_train= x_train.reshape(36724*1, 9)
x_val= x_val.reshape(15740*1, 9)
...
x_train= x_train.reshape(36724, 1, 9)
x_val= x_val.reshape(15740, 1, 9)
x_test= x_test.reshape(3888, 1, 9)
...
lr = ReduceLROnPlateau(monitor = 'val_loss', patience = 6, factor = 0.25, verbose = 1)
epochs = ep = 200
batch_size = ba = 64
...
for i in q:
    model = modeling()
    filepath_cp = f'C:/data/modelCheckpoint/dacon_y1_0124_1_q_{i:.1f}.hdf5'
...
for i in q:
    model = modeling()
    filepath_cp = f'C:/data/modelCheckpoint/dacon_y2_0124_1_q_{i:.1f}.hdf5'
    cp = ModelCheckpoint(filepath_cp,save_best_only=True,monitor = 'val_loss')
    model.compile(loss = lambda y_true,y_pred: quantile_loss(i,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(i,y,y_pred)])
...
submission.to_csv('C:/data/dacon_data/sub_0124_1.csv', index = False)
