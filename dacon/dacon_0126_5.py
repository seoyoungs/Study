...
#### temp 에 T 넣기
    ...
    # df["ordering"] = (df["Day"] // 4)
#https://dacon.io/competitions/official/235680/codeshare/2301?page=1&dtype=recent&ptype=pub
...
def Add_features(data):
    data.insert(1,'Hour_Minute',data['Hour'] * 2 + data['Minute'] // 30)
    data.insert(1,'ordering',(data['Day'] // 4))
...
def preprocess_data(data, is_train=True):
    ...
    temp = data.copy()
    temp = temp[['Hour','TARGET','GHI','T-Td','DHI','DNI', 'RH','T']] # 상관관계 낮은 WS, T 삭제
    ...
    if is_train==True:          
        ...
        temp = temp[['Hour','TARGET','GHI','T-Td','DHI','DNI', 'RH','T']]
...
x = df_train.iloc[:,:-2]
y1 = df_train.iloc[:,-2:-1]
y2 = df_train.iloc[:,-1:]
# print(x.shape, y1.shape, y2.shape) #(52464, 8) (52464, 1) (52464, 1)
...
x = split_x(pd.DataFrame(x), 1)
y1 = split_x(y1, 1)
y2 = split_x(y2, 1)
# print(x.shape, y1.shape, y2.shape) #(52464, 1, 8) (52464, 1, 1) (52464, 1, 1)
...
#####===================================================전처리
x_train, x_val, y1_train, y1_val = train_test_split(x,y1, train_size = 0.7,
                                        shuffle = False, random_state = 0)
x_train, x_val, y2_train, y2_val = train_test_split(x,y2,train_size = 0.7,
                                        shuffle = False, random_state = 0)

print(x_train.shape,x_val.shape) #(41971, 1, 7) (10493, 1, 8)
...
# print(x_train.shape,x_val.shape,x_test.shape) #(41971, 1, 7) (10493, 1, 7) (3888, 1, 7)
...
#####========컴파일,  훈련, predict
#y1, y2 각각 저장하기
es = EarlyStopping(monitor = 'val_loss', patience = 11, mode='min')
...
for i in q:
    model = modeling()
    filepath_cp = f'C:/data/modelCheckpoint/dacon_y1_0126_5_q_{i:.1f}.hdf5'
    cp = ModelCheckpoint(filepath_cp, monitor='val_loss', mode='min', verbose=1, save_best_only=True, save_weights_only=True)
    ...
    model.fit(x_train,y1_train,epochs = ep, batch_size = ba, validation_data = (x_val,y1_val),callbacks = [es,cp,lr])
...
for i in q:
    model = modeling()
    filepath_cp = f'C:/data/modelCheckpoint/dacon_y2_0126_5_q_{i:.1f}.hdf5'
    cp = ModelCheckpoint(filepath_cp, monitor='val_loss', mode='min', verbose=1, save_best_only=True, save_weights_only=True)
    model.compile(loss = lambda y_true,y_pred: quantile_loss(i,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(i,y,y_pred)])
...
submission.to_csv('C:/data/dacon_data/sub_0126_5.csv', index = False)
