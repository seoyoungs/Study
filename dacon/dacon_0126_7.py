...
def hahaha(a, x_train, y_train, x_val, y_val, x_test):
    x = []
    for q in quantiles:
        filepath_cp = f'C:/data/modelCheckpoint/solar_checkpoint9_time11-{a}-{q}.hdf5'
...
df.loc[:,:] = data
df.to_csv('C:/data/dacon_data/train/train_trans.csv', index=False)
# 시간별 모델 따로 생성
train_trans = pd.read_csv('C:/data/dacon_data/train/train_trans.csv')
...
sub.to_csv('C:/data/dacon_data/sub_0126_7.csv', index=False)
