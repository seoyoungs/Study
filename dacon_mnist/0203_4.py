...
# 이미지 생성기의 선언
idg = ImageDataGenerator(width_shift_range=5,
                         height_shift_range=5,
                         rotation_range=10,
                         zoom_range=0.05)

#ImageDataGenerator 머신러닝
...
re = ReduceLROnPlateau(patience=80, verbose=1, factor= 0.5)
ea = EarlyStopping(patience=120, verbose=1, mode='auto')
epochs = 1000
#KFlod대신 StratifiedKFold 써보기
#stratified 는 label 의 분포를 유지, 각 fold가 전체 데이터셋을 잘 대표한다.
kf = KFold(n_splits=20, random_state=42, shuffle=True) #n_splits 몇 번 반복
val_loss_min = []
# val_accuracy_min = []
y_pred = 0
n = 0
...
for train_index, valid_index in kf.split(train2, t_d):
    ...
    train_generator = idg.flow(x_train,y_train,batch_size=32,seed=2020) #훈련데이터셋을 제공할 제네레이터를 지정
    valid_generator = idg.flow(x_valid,y_valid,batch_size=32,seed=2020) # validation_data에 넣을 것
    test_generator = idg.flow(test2,batch_size=32,seed=2020,shuffle=False)  # predict(x_test)와 같은 역할
    
    model = modeling()
    mc = ModelCheckpoint('C:/data/modelCheckpoint/0203_4_best_mc.h5', save_best_only=True, verbose=1)
...
sub
sub.to_csv('C:/data/dacon_mnist/answer/0203_4_mnist.csv',index=False)
