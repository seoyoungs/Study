...
# 모델
model = XGBClassifier(n_jobs=-1, use_label_encoder=False) #use_label_encoder 문자형-> 숫자로 안바꾼다

# 훈련
model.fit(x_train,y_train, eval_metric='logloss') # metrics를 에러말고 log로 쓴다
y = dataset.target
# 평가, 예측
...
# 훈련
model.fit(x2_train,y2_train, eval_metric='logloss')
y = dataset.target
