...
# w1 = Conv2D(64, (3,3), input_shape = (32, 32, 3))   # 파라미터 개수? 70개
# 커널 사이즈 곱하기 채널 * 인풋 딤 
...
# -------------- L2
w2 = tf.compat.v1.get_variable('w2', shape=[3, 3, 128, 64]) # 첫번째 아웃풋이 두번째 인풋  
...
w5 = tf.compat.v1.get_variable('w5', shape = [2*2*64, 64],
                      initializer = tf.keras.initializers.he_normal()) # he_initializer
...
# --------------- L6
w6 = tf.compat.v1.get_variable('w6', shape = [64, 16],
                      initializer = tf.keras.initializers.he_normal()) # he_initializer
b6 = tf.Variable(tf.compat.v1.random_normal([16], name = 'b6'))
L6 = tf.nn.selu(tf.matmul(L5, w6)+ b6)
# L6 = tf.nn.dropout(L6, keep_prob=0.2)
print(L6)

# --------------- L7(출력 레이어)
w7 = tf.compat.v1.get_variable('w7', shape = [16, 10],
                      initializer = tf.keras.initializers.GlorotNormal()) # he_initializer
...
optimizer = tf.compat.v1.train.AdamOptimizer(1e-5).minimize(loss)
